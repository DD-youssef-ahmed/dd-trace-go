...
func spanStart(operationName string, sharedAttrs *traceinternal.SpanAttributes, poolEnabled bool, options ...StartSpanOption) *Span {
	...
	if context != nil && !context.baggageOnly { // +checklocksignore - Read-only after init.
		...
		if !context.local && context.origin != "" { // +checklocksignore - Read-only after init.
			// mark origin
	...
	isRootSpan := context == nil || !context.local
...
func (t *tracer) StartSpan(operationName string, options ...StartSpanOption) *Span {
	...
	if span.service == "" {
		...
		span.syncContextInheritedLocked()
	}
	...
	if cSnap.Version != "" {
		if t.config.universalVersion || span.service == cSnap.ServiceName {
			span.setMetaInit(ext.Version, cSnap.Version)
		}
	}
	if cSnap.Env != "" {
		span.setMetaInit(ext.Environment, cSnap.Env)
	}
