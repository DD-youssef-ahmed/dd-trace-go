...
// RuntimeProducer implements sdkmetric.Producer for OTel Go runtime histogram
// metrics that runtime/metrics exposes as Float64Histogram distributions.
//
// Currently emits only go.schedule.duration (Recommended). The SDK's observable
// callback path cannot emit Histograms, so a Producer is the SDK-blessed way
// to bridge cumulative bucket-count snapshots into OTel histogram data points.
//
