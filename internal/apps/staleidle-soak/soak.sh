...
case "$SCENARIO" in
  ...
  *) echo "unknown scenario: $SCENARIO (want B.1 / B.2 / B.3)" >&2; exit 2 ;;
esac
...
if [ "$USE_STALE_PROXY" = "1" ]; then
  ...
  echo "[soak] starting stale-conn proxy: $PROXY_SOCKET -> $AGENT_SOCKET (close after ${CLOSE_AFTER_RESP} response)"
  "$PROXY_BIN" \
    --listen "$PROXY_SOCKET" \
    --target "$AGENT_SOCKET" \
